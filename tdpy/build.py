...
ENGINE_PROGRAM_TOP = "engineProgram"
...
def build():
    ...
    from . import lister, midi, player, settings, startup
    ...
    # Same reasoning, for the controller's lamps: a launch is not a change, so
    # no watcher fires and the LEDs would sit at whatever the device powered up
    # with until the first pause or cut.
    midi.refresh_lights()

    # Last, and reported rather than raised. Until 9.4 the engine runs a probe
    # beside the working player rather than instead of it, so a fault here
    # should cost the probe and leave the player playing - the same trade
    # `startup._add_controls` makes for the rebuild button.
    try:
        ...
        startup.report(
            f"[{startup.PACKAGE}] engine setup failed\n{traceback.format_exc()}"
        )
...
def _add_engine(parent, td):
    ...
    The `.tox` carries the bootstrap and the Out operators - its interface -
    and nothing behind them. The network behind the outputs is built inside
    the engine by `tdpy.engine.main()`, which is what makes an edit to that
    module arrive with a rebuild rather than a regenerated file.

    The Engine COMP is converged onto: created on the first build, then given
    the same file and pulsed to reload. Its callbacks DAT and the host's Null
    exist before its `file` is set, because setting it on a new COMP is what
    starts the load, and `onReady` reaches for both.
    """
    ...
    program = link.output("program_video")
    out = startup.create(source, td.outTOP, program.name)
    startup.set_par(out, "label", program.label)
    ...
    if parent.op(ENGINE_PROGRAM_TOP) is None:
        _place(startup.create(parent, td.nullTOP, ENGINE_PROGRAM_TOP), 450, -300)
...
def wire_engine(comp):
    """Connect the engine's program video to the host's Null. Idempotent.

    Called by the Engine COMP's `onReady`, which fires after each load. It says
    what it did either way, since the host's first sight of the engine working
    is this line and its first sight of it not working is its absence.
    """
    from . import link, startup

    target = comp.parent().op(ENGINE_PROGRAM_TOP)
    if target is None:
        startup.report(f"[{startup.PACKAGE}] no {ENGINE_PROGRAM_TOP} beside {comp.path}")
        return None

    label = link.output("program_video").label
    connector = connector_for(comp.outputConnectors, label)
    if connector is None:
        found = ", ".join(repr(c.description) for c in comp.outputConnectors)
        startup.report(
            f"[{startup.PACKAGE}] no engine output labelled {label!r}"
            f" - have {found or 'none'}"
        )
        return None

    connector.connect(target)
    startup.report(f"[{startup.PACKAGE}] engine {label} -> {target.path}")
    return target
